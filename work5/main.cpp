...
void task2();
double sum(double t);
double func(complex<double>* signal, int size, double t);
...
void task1() {
	...
	for (int i = 0; i < size; i++) {
		double t = (i + 1) / 128.;
		inputSignal[i] = complex<double>(sin(2. * PI * 10 * t));
	}
	ParallelFFT(inputSignal, outputSignal, size);
	cout << "Здание 1" << endl;
	for (int i = 0; i < size; i++) {
		if (abs(outputSignal[i].real()) > eps && abs(outputSignal[i].imag()) > eps) {
			cout << i << " " << outputSignal[i] << "\n";
		}
	}
	double norm = sqrt(outputSignal[10].real() * outputSignal[10].real() + outputSignal[10].imag() * outputSignal[10].imag());
	norm /= size / 2;
}
...
void task2() {
	...
	for (int i = 1; i < size; i++) {
		double t = i * 1.0 / size;
		inputSignal[i] = complex<double>(-1.0 * log(2 * sin((PI / T) * t)));
	}
	...
	cout << "Задание 2" << endl;
	cout << "Значения" << "Коэф.Фурье" << "Вычисленные значения" << endl;
	for (int i = 1; i < 64; i++) {
		double t = i * 1.0 / size;
		cout << setw(8) << func(outputSignal, size, t) << " " << setw(8) << sum(t) << " " << setw(8) << PI / 2. - PI * t / T << endl;
	}
}

double sum(double t) {
	double eps = 1e-9, res = 0, s = 0;
	int k = 1;
	do {
		s = sin(k * 2. * PI / T * t) / k;
		res += s;
...
double func(complex<double>* signal, int size, double t) {
	...
	for (int i = 1; i < size / 2; i++) {
		res += signal[i].real() * cos(i * 2. * PI / T * t) + signal[i].imag() * sin(i * 2. * PI / T * t);
	}
	return res;
}
